...
const Dashboard = () => {
  ...
  const fetchAnalytics = async (shortCode) => {
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/urls/analytics/${shortCode}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDelete = async (shortCode) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/urls/${shortCode}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <main className="min-h-screen flex flex-col">
        ...
        {urls.length > 0 && (
          <section className="p-6">
            ...
            <BarChart
              ...
              series={[
                { dataKey: "clicks", label: "Clicks", color: "#2563eb" },
              ]}
              height={300}
            />
          </section>
        ...
        <section className="p-6">
          <h3 className="font-lg font-semibold mb-3 text-gray-800">
            Your URLs
          </h3>
          {urls.length === 0 ? (
            ...
            <ul>
              {urls.map((u) => (
                ...
                >
                  ...
                  <button
                    onClick={() => fetchAnalytics(u.shortCode)}
                    className="bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-1 rounded mt-2 sm:mt-0"
                  >
                    Analytics
                  </button>
                  <button
                    onClick={() => handleDelete(u.shortCode)}
                    className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded mt-2 sm:mt-0"
                  >
                    Delete
                  </button>
                </li>
          ...
          <div className="fixed inset-0 flex flex items-center justify-center bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded-lg max-w-lg w-full max-h-[80vh] overflow-y-auto shadow-lg">
              ...
              <p className="mb-2">
                ...
                {selectAnalytics.totalClicks}
              </p>
              <p className="mb-2">
                <strong>Unique Visitors:</strong>
                {selectAnalytics.uniqueVisitors}
              </p>
              ...
              <ul className="list-disc list-inside max-h-32 overflow-y-auto">
                {selectAnalytics.dailyClicks.map((d, idx) => (
                  <li key={idx}>
                    {new Date(d.createdAt).toLocaleString()} - {d._count} click{" "}
                    {d._count > 1 ? "s" : ""}
                  </li>
                ))}
              </ul>
